...
from frappe.utils import cstr, comma_and, cint, nowdate
...
class LMSQuiz(Document):
    ...
    def validate_duplicate_questions(self):
        ...
        if len(rows):
            frappe.throw(
                _("Rows {0} have the duplicate questions.").format(
    ...
    def validate_limit(self):
        if self.limit_questions_to and cint(self.limit_questions_to) >= len(
            ...
            frappe.throw(
                _(
                    "Limit cannot be greater than or equal to the number of questions in the quiz."
                )
            )
        ...
        if self.limit_questions_to and cint(self.limit_questions_to) < len(
            ...
            if len(set(marks)) > 1:
                frappe.throw(
                    _("All questions should have the same marks if the limit is set.")
                )
...
def quiz_summary(quiz, results):
    ...
    percentage = (score / score_out_of) * 100 if score_out_of > 0 else 0
    ...
    if percentage >= quiz_details.passing_percentage:
        ...
        certificate = frappe.db.get_value(
            ...
            ["name", "member", "course", "course_title", "issue_date", "template"],
            as_dict=True,
        )
...
def update_questions(quiz, questions):
    ...
    frappe.db.set_value(
        "LMS Quiz", quiz, "total_marks", set_total_marks(quiz, questions)
    )
