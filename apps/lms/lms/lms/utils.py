...
def get_course_details(course):
    ...
    if course_details.paid_course:
        ...
        course_discount = (
            course_details.discount_price if course_details.discount_price else 0
        )
        course_details.price = fmt_money(
            course_details.course_price - course_discount, 0, course_details.currency
        )
...
def get_order_summary(doctype, docname, country=None):
    ...
    course_discount = details.discount_price if details.discount_price else 0
    details.discount_price = fmt_money(course_discount, 0, details.currency)
    details.amount, details.currency = check_multicurrency(
        details.amount - course_discount, details.currency, country, details.amount_usd
    )
    ...
    details.amount = fmt_money(details.amount + course_discount, 0, details.currency)
    return details
...
def get_lesson_creation_details(course, chapter, lesson):
    ...
    # Fetch the lesson name
    lesson_name = frappe.db.get_value(
        "Lesson Reference", {"parent": chapter_name, "idx": lesson}, "lesson"
    )
    # Get chapter details
    chapter_obj = frappe.db.get_value(
    ...
    if lesson_name:
        ...
        # Parse the content field to extract quiz details
        if lesson_details and lesson_details.get("content"):
            try:
                content_data = frappe.parse_json(lesson_details["content"])
                blocks = content_data.get("blocks", [])

                # Check for quiz type blocks
                for block in blocks:
                    if block.get("type") == "quiz":
                        quiz_key = block.get("data", {}).get("quiz")
                        if quiz_key:
                            # Fetch quiz details from LMS Quiz doctype
                            quiz_doc = frappe.get_doc("LMS Quiz", quiz_key)
                            quiz_doc.lesson = lesson_name
                            quiz_doc.save(ignore_permissions=True)
            except Exception as e:
                frappe.log_error(
                    f"Error processing lesson content: {e}", "Lesson Quiz Parsing"
                )
