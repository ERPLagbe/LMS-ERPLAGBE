<template>
	<div class="shadow rounded-md min-w-80">
		...
		<div class="p-5">
			<div
				v-if="course.data.discount_price > 0"
				class="font-semibold line-through text-red-500"
			>
				৳ {{ course.data.course_price }}
			</div>
			...
			>
				<button
					class="w-full bg-[#c2e31fd0] hover:bg-[#c2e31f] px-5 py-2 rounded-md text-[#004341]"
				>
					<span>
						{{ __('Buy this course') }}
					</span>
			...
			<a
				v-else-if="course.data.paid_course"
				:href="`/lms/billing/course/${course.data.meta_title}`"
			>
				<button
					class="w-full bg-[#c2e31fd0] hover:bg-[#c2e31f] px-5 py-2 rounded-md text-[#004341]"
				>
